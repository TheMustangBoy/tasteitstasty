...
BEGIN
  ...
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_lines) LOOP
    ...
    -- Basispreis des Clients muss dem hinterlegten Produktpreis entsprechen
    IF (v_line ? 'basePrice')
       AND abs(v_product.price - COALESCE((v_line->>'basePrice')::numeric, -1)) > 0.005 THEN
      RAISE EXCEPTION 'PRICE_CHANGED';
    END IF;

    -- Abgewählte Zutaten müssen im Produkt als abwählbar hinterlegt sein
    ...
    ELSIF jsonb_typeof(COALESCE(v_line->'variant', 'null'::jsonb)) = 'object' THEN
      -- Legacy-Warenkorb: einzelne Variante wie eine Auswahl-Option behandeln
    ...
    -- Normalisierter Snapshot: Preise stammen ausschließlich aus der Datenbank
    ...
    v_norm_lines := v_norm_lines || v_norm_line;
  END LOOP;
  ...
  VALUES (p_reference, p_customer_name, p_phone, p_pickup_at, p_pickup_label,
          p_payment, v_norm_lines, v_calc_total, COALESCE(p_note, ''))
